...
- **功能**
  ...
  - **注意**：风险备付金明细弹窗为只读模式，显示汇总数据（只读），不支持修改汇总和新增明细
  ...
  - 回购限制编辑功能：
    ...
    - 点击后打开回购限制编辑弹框
    - **数据回显**：打开弹框时，需要从当前行数据中获取回购限制相关字段值（`repurchaseRestrictionType`、`pledgeRepoLimit`、`pledgeRepoBondLimit`、`reverseRepoLimit`）回显到弹框表单中
...
- **汇总输入框**：显示点击的表格数值（即该单元格的值），用户可修改汇总值并保存
  ...
  - **风险备付金特殊说明**：风险备付金明细弹窗显示汇总数据（只读），不支持修改汇总和保存，不显示保存按钮，仅支持查看明细数据
- **明细表格**（证券代码、证券名称、业务类别、交易金额、是否交收）
  ...
  - **风险备付金特殊说明**：风险备付金明细弹窗不支持新增明细，不显示"新增"按钮，仅支持查看明细数据
  - **新增行数据字段要求**：
...
- 正回购不可押下拉框（单选）
  ...
  - 参数：`dictId: "POSITION_BOND_TYPE"` // 头寸债券类型
  - 下拉框配置：单选、可搜索、可清空
- 正回购自定义不可押文本输入框
  - 支持用户输入文本内容
- 逆回购可押下拉框（单选）
...
- 取消、保存按钮
- **数据回显说明**
  - 点击回购限制单元格时，需要从表格数据中获取对应字段值回显到弹框表单中
  - 回显字段来源：查询接口 `/prodPosition/query` 返回的数据（参考 @position-planning-api.md 第1节）
  - 回显字段映射：
    - 期限限制：`repurchaseRestrictionType`
    - 正回购不可押：`pledgeRepoLimit`
    - 正回购自定义不可押：`pledgeRepoBondLimit`
    - 逆回购可押：`reverseRepoLimit`
- **保存接口参数说明**
  - 接口：`/prodPositionRepurLimit/save`（参考 @position-planning-api.md 第2.2节）
  - 参数：
    - `productCode`: 产品代码（必传）
    - `repurchaseRestrictionType`: 回购期限限制（如：'GY'、'7D'、'14D'）
    - `pledgeRepoLimit`: 正回购不可押（下拉框选中的值）
    - `pledgeRepoBondLimit`: 正回购自定义不可押（文本输入框的值）
    - `reverseRepoLimit`: 逆回购可押（下拉框选中的值）
...
- **风险备付金明细弹窗**：
  - 点击风险备付金列，弹框显示明细
  - 汇总数据：显示汇总金额（只读显示，不可编辑）
  - 明细数据展示：显示该字段对应的明细列表（同正回购到期列的明细展示方式）
  - **不支持修改汇总和新增明细**：此明细弹框只支持查看，汇总数据为只读显示，不显示保存按钮和"新增"按钮，仅显示明细表格和取消按钮
